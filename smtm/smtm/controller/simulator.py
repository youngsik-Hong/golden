...
class Simulator:
    ...
    def __init__(
        ...
        self.command_list = [
            {"guide": "h, help          print command info", "cmd": ["help", "h"], "action": self.print_help},
            {"guide": "r, run           start running simulation", "cmd": ["run", "r"], "action": self.start},
            {"guide": "s, stop          stop running simulation", "cmd": ["stop", "s"], "action": self._stop},
            {"guide": "t, terminate     terminate simulator", "cmd": ["terminate", "t"], "action": self.terminate},
            {"guide": "i, initialize    initialize simulation", "cmd": ["initialize", "i"], "action": self.initialize_with_command},
            {"guide": "1, state         query operating state", "cmd": ["1"], "action": self._print_state},
            {"guide": "2, score         query current score", "cmd": ["2"], "action": self._print_score},
            {"guide": "3, result        query trading result", "cmd": ["3"], "action": self._print_trading_result},
        ]

        all_strategy = StrategyFactory.get_all_strategy_info()
        strategy_guide = ", ".join([item["code"] for item in all_strategy])
        strategy_guide = "전략 코드 입력. " + strategy_guide
        self.config_list = [
            {"guide": "년월일.시분초 형식으로 시작 시점 입력. 예. 201220.162300", "value": self.start_str, "action": self._set_start_str},
            {"guide": "년월일.시분초 형식으로 종료 시점 입력. 예. 201220.162300", "value": self.end_str, "action": self._set_end_str},
            {"guide": "거래 간격 입력(초). 예. 60", "value": self.interval, "action": self._set_interval},
            {"guide": "예산 입력. 예. 50000", "value": self.budget, "action": self._set_budget},
            {"guide": strategy_guide, "value": self.strategy, "action": self._set_strategy},
            {"guide": "화폐 코드 입력. BTC, ETH", "value": self.currency, "action": self._set_currency},
        ]
    ...
    def initialize(self):
        ...
        if self.operator is None or not isinstance(self.operator, SimulationOperator):
        ...
        if self.fast:
            self.operator.set_interval(0.0)
        else:
    ...
    def run_single(self):
        ...
        if self.operator is None:
            raise RuntimeError("SimulationOperator is not created.")
        if getattr(self.operator, "state", None) != "running":
            self._print("Simulation did not start (operator.state != 'running')")
            self.terminate()
            return

        fast_mode = self.fast
        tiny_sleep = 0.001 if fast_mode else 0.5
        max_wait_sec = 45 if fast_mode else None
        start_ts = time.time()

        while self.operator is not None and self.operator.state == "running":
            time.sleep(tiny_sleep)
            if fast_mode and max_wait_sec is not None:
                if (time.time() - start_ts) > max_wait_sec:
                    try:
                        self.logger.warning(f"Fast simulation watchdog timeout ({max_wait_sec}s). Forcing stop.")
                    except Exception:
                        pass
                    try:
                        self.operator.stop()
