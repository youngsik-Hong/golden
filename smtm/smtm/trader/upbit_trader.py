...
class UpbitTrader(Trader):
    ...
    @staticmethod
    def _create_limit_order_query(market, is_buy, price, volume, identifier=None):
        query = {
            "market": market,
    ...
    @staticmethod
    def _create_market_price_order_query(market, price=None, volume=None, identifier=None):
        query = {
            "market": market,
        }
        if identifier:
    ...
    def _execute_order(self, task):
        ...
        identifier = request.get("identifier") or request.get("client_oid") or request.get("id")
        response = self._send_order(
            self.market, is_buy, request["price"], request["amount"], identifier=identifier
        )
        if response is None:
            task["callback"]("error!")
            return
    ...
    def _send_order(self, market, is_buy, price=None, volume=None, identifier=None):
        """
        Upbit에 거래 주문 전송
        ...
        if price is not None and volume is not None:
            ...
            query_string = self._create_limit_order_query(
                market, is_buy, final_price, volume, identifier=identifier
            )
        elif volume is not None and is_buy is False:
            # 시장가 매도
            self.logger.warning("### Marker price order is submitted ###")
            query_string = self._create_market_price_order_query(market, volume=volume, identifier=identifier)
        elif price is not None and is_buy is True:
            # 시장가 매수
            self.logger.warning("### Marker price order is submitted ###")
            query_string = self._create_market_price_order_query(market, price=price, identifier=identifier)
        else:
            # 잘못된 주문
    ...
    def _query_order_list(self, uuids, is_done_state=True, state=None):
        """
        Upbit에 주문 리스트 요청
        ...
        response:
            ...
            trade_count: 해당 주문에 걸린 체결 수, Integer
        """
        # state 인자를 명시하면 해당 상태만 조회 (Upbit 주문조회 파라미터 호환)
        if state:
            query_states = [state]
        else:
    ...
    def _query_orders(self, state="wait", market=None):
        """주문 목록 조회 (호환용).

        - state: "wait"/"watch"/"done"/"cancel"
        - market: 현재는 미사용(호환 목적).

        Upbit 주문조회 API는 state 기반 조회를 지원하므로,
        내부적으로 _query_order_list(state=...)로 위임한다.
        """
        # uuids 기반 조회 로직을 그대로 활용 (uuids는 빈 리스트)
        return self._query_order_list([], is_done_state=False, state=state)
    ...
    def get_order(self, request_uuid=None, identifier=None):
        """Upbit 단일 주문 조회 (/v1/order). uuid 또는 identifier 중 하나를 사용."""
        query = {}
        if request_uuid:
        ...
        authorize_token = "Bearer {}".format(jwt_token)
        headers = {"Authorization": authorize_token}
        return self._request_get(self.SERVER_URL + "/v1/order", params=query_string, headers=headers)

    def reconcile_for_engine(self, client_oid, exchange_order_id=None):
        """엔진(OrderManager)용 재조회 결과를 표준 dict로 변환."""
        data = None
        if exchange_order_id:
            data = self.get_order(request_uuid=exchange_order_id)
        if data is None:
            data = self.get_order(identifier=client_oid)
        if not isinstance(data, dict):
            return None

        state = (data.get("state") or "").lower()
        executed = float(data.get("executed_volume") or 0.0)
        remaining = float(data.get("remaining_volume") or 0.0) if data.get("remaining_volume") is not None else None
        paid_fee = float(data.get("paid_fee") or 0.0)

        # avg_price 계산: trades 기반 가중평균. 없으면 주문 price 사용
        avg_price = None
        ...
        return {
            "exchange_order_id": data.get("uuid") or exchange_order_id,
            "state": state,
